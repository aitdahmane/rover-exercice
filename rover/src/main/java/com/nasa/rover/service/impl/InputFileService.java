...
public class InputFileService implements IInputFileService {
    ...
    private static final Pattern ROVER_POSITION_PATTERN = Pattern.compile("^\\d+\\s+\\d+\\s+[NESW]$");
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(new File(filePath)))) {
            ...
            while ((positionLine = reader.readLine()) != null) {
                ...
                if (!plateau.isValidPosition(x, y)) {
                    throw new Exception("Rover position is outside of plateau boundaries: " + x + " " + y);
                }
                ...
                try {
                    ...
                    throw new Exception("Invalid direction: " + directionValue);
                }
